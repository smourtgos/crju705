...
The tidyverse plus **BayesFactor**, the two groups from Session 5 (built the same way: Violent first, so every difference reads Violent minus Property), and Session 5's little `clock()` helper for axis labels:
...
clock <- function(h) {   # hours after midnight -> "12:48 pm"
  m  <- round(h * 60)
  hh <- (m %/% 60) %% 24
  mm <- m %% 60
...
Violent incidents average about **12:48 pm**, property about **12:33 pm**: roughly **15 minutes later**. Session 5 gave us an interval for that gap. Today we get the *test* — a yes/no verdict machine — and then a second tradition that answers the question you probably *meant* to ask.
...
boot |>
  ...
  mutate(group = factor(group, levels = c("Violent", "Property"))) |>
  ...
  labs(title = "10,000 bootstrap means per group",
       x = "Mean time of day", y = "Count", fill = NULL)
```

Subtract, pair by pair, and the *difference* gets its own distribution. We work in minutes from here on:

```{r}
#| label: boot-diff
#| fig-height: 4.5
boot_diff <- (boot$Violent - boot$Property) * 60   # in minutes: Violent minus Property
...
ggplot(tibble(boot_diff), aes(boot_diff)) +
  ...
  labs(title = "Bootstrap distribution of the difference (Violent minus Property)",
       subtitle = "Red line = zero (no difference); gold dashes = 95% interval",
       x = "Difference in mean time of day, minutes", y = "Count")
```

The middle 95% of plausible differences runs from about **`r round(ci_boot[1])` to `r round(ci_boot[2])` minutes** — zero is not among the plausible values. The data lean toward a real (if small) gap.
...
boot_diff_null <- replicate(10000,
  ...
  mean(sample(property_hours,  replace = TRUE))) * 60   # minutes

ci_null <- quantile(boot_diff_null, c(0.025, 0.975))
ci_null
```

Now the interval runs from about **`r round(ci_null[1])` to +`r round(ci_null[2])` minutes** — zero sits comfortably inside, and the data are *consistent with no difference*. (Session 5's arrest-versus-no-arrest comparison was the real-data version of this picture.) Formalize the intuition and you have invented the NHST.
...
Reading the verdict: Violent is listed first because we told R so (the `factor()` line from Session 5), so this tests **Violent minus Property** — the same 4-to-27-minute interval as our bootstrap, now with a test attached. $t = 2.61$, $p = .009$. Under .05 — **"statistically significant."** Reject the null. Publish the press release?
...
diff_post <- as.numeric(post[, "beta (x - y)"]) * 60   # in minutes: Violent minus Property
...
ggplot(tibble(diff_post), aes(diff_post)) +
  ...
  labs(title = "Posterior distribution of the difference (Violent minus Property)",
       subtitle = paste0("95% credible interval: ", round(cred[1]), " to ",
                         round(cred[2]), " minutes"),
       x = "Difference in mean time of day, minutes", y = "Posterior samples")
```

Now say the sentence Session 5 forbade — **legally**: *"There is a 95% probability the true difference lies between about `r round(cred[1])` and `r round(cred[2])` minutes."* Under the Bayesian license (given the model and prior), that is exactly what this **credible interval** means. And about `r round(100 * mean(diff_post > 0), 1)`% of the posterior sits above zero — the difference is almost certainly *real*. Real, but…
...
1. Run both tests on a new question: does `hour` differ for **domestic vs. non-domestic** incidents? Build the grouping variable first, the Session 5 way, so you choose which group comes first: `dom <- crimes |> mutate(domestic = factor(if_else(domestic, "Domestic", "Not domestic"), levels = c("Domestic", "Not domestic")))`, then `t.test(hour ~ domestic, data = dom)` and `ttestBF(formula = hour ~ domestic, data = dom)`. Do the two traditions agree this time?
