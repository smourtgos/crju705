...
# Same two groups as Session 5, Violent first, so every difference reads Violent minus Property
vp <- crimes |>
...
# hours after midnight -> clock time, for axis labels (as in Session 5)
clock <- function(h) {
  m  <- round(h * 60)
  hh <- (m %/% 60) %% 24
  mm <- m %% 60
...
Lab 6: joining the two Chicago files with `left_join()`, step by step
:::
::::
...
- **Session 5:** confidence intervals — an honest range; our violent-vs-property gap came out *detectable but tiny* (≈4–27 min)
...
Same two groups, Violent first, so every difference today reads **Violent minus Property**. Violent incidents average about **12:48 pm**, property about **12:33 pm**: roughly **15 minutes later**. Session 5 gave us an interval for that gap. Today we get the *test* — a yes/no verdict machine.
...
The middle 95% of plausible differences runs from about **`r round(ci_boot[1])` to `r round(ci_boot[2])` minutes** — the interval sits *entirely above zero*.
...
ggplot(tibble(boot_diff_null), aes(boot_diff_null)) +
  ...
  labs(title = "Same machinery, after erasing the gap",
       subtitle = paste0("95% interval: ", round(ci_null[1]), " to ",
                         round(ci_null[2]), " minutes; zero is now a plausible value"),
       x = "Difference in mean time of day, minutes", y = "Count")
```

When zero sits comfortably inside, the data are *consistent with no difference* (Session 5's arrest-versus-no-arrest picture was the real-data version of this). Formalize the intuition and you have invented the NHST.
...
- Violent is listed first because we told R so (Session 5's `factor()` line), so this tests **Violent minus Property**: the same 4-to-27-minute interval as our bootstrap, now with a *t* and a *p* attached
...
Same question, second tradition. We hand it the two groups ourselves, Violent first again: **x − y = Violent minus Property**.
...
- Rules of thumb: 1–3 barely worth mentioning · 3–10 moderate · 10–30 strong · 30–100 very strong · 100+ extreme (mirror image below 1)
:::
...
ggplot(tibble(diff_post), aes(diff_post)) +
  ...
  labs(title = "Posterior distribution of the difference (Violent minus Property)",
       subtitle = paste0("95% credible interval: ", round(cred[1]), " to ",
                         round(cred[2]), " minutes · P(difference > 0) = ",
                         round(p_gt0, 3)),
...
Now say the sentence Session 5 forbade — **legally**: *"There is a 95% probability the true difference lies between about `r round(cred[1])` and `r round(cred[2])` minutes."* Under the Bayesian license (given the model and prior), that is exactly what this interval means.
...
- **Bayes** answers $P(H \mid \text{data})$ at the price of a stated prior; MCMC made it computable (appendix, if you want the machinery)
...
- **Lab 6:** joining data — `left_join()` on the two Chicago files, and the ways a join quietly goes wrong ([lab page](../labs/lab-06-joins.qmd))
...
*Not on any exam — this is the machinery behind the two-second `ttestBF()` call, for the curious.*
