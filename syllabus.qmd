...
# from these values.
# ---------------------------------------------------------------------------
firstday      <- ymd("2026-08-19")   # first Wednesday class meeting (PROVISIONAL)
semester_start <- ymd("2026-08-18")  # PROVISIONAL
semester_end   <- ymd("2026-12-05")  # PROVISIONAL

not_here_dates <- c(
  # Instructor at conference (ASC) — PROVISIONAL
  ymd("2026-11-18"),
  # Fall Break — PROVISIONAL (Thu–Fri, does not hit a Wednesday)
  seq(ymd("2026-10-15"), ymd("2026-10-16"), by = 1),
  # Thanksgiving Break — PROVISIONAL
  seq(ymd("2026-11-22"), ymd("2026-11-29"), by = 1)
)

# All Wednesdays in the semester
all_weds <- seq(firstday, semester_end, by = "1 week")
# Class sessions = Wednesdays not blocked by breaks/conference
session_dates <- all_weds[!all_weds %in% not_here_dates]

exam_dates <- c(session_dates[9], session_dates[14])  # midterm, final presentations
final_day  <- session_dates[14]
...
**Provisional dates.** Session dates on this page derive from a provisional Fall 2026 calendar and will be finalized before the semester begins. Topics, their order, and course policies are final.
:::
...
**Class meetings:** Wednesdays, 5:50–8:35 pm · Currell 204
...
This course requires consistent engagement and practice. You are expected to attend and actively participate in class sessions, complete in-class exercises in R, and contribute to discussions. Assigned readings — from the Stanton textbook and from *R for Data Science* — prepare you for lectures and hands-on demonstrations. You will complete in-class exercises and submit R scripts and output from weekly homework assignments. Beginning mid-semester, several homework assignments include a **project checkpoint**: a short component where you apply that week's technique to the dataset you selected for your final project, so the project builds incrementally rather than all at once. The course concludes with a final project that integrates conceptual understanding with applied statistical skills in R.
...
The final project is the culminating assignment for this course, integrating conceptual understanding with applied statistical skills in R. You will select your own dataset early — **dataset selection is due at Session 7** (the applied data workshop) — and project checkpoints attached to homework in the second half of the semester keep your analysis moving. A dedicated **help session (Session 13)** provides open Q&A and a code clinic before presentations.

Your completed project — R script, output, and written report — is due **via Canvas by 11:59 pm on Sunday, `r format(final_day - days(3), '%B %d, %Y')`** (the Sunday before presentations day). You will then present your work in class at Session 14 on `r format(final_day, '%B %d, %Y')`.
...
**Homework:** swirl tutorials in R (instructions in class). Submit R script files via Canvas **before noon Tuesday** ahead of the next session. Read Stanton Ch. 1 before next class.
...
**Topics:** **Final-project help session** — open Q&A, code clinic, project check-ins

**No in-class exercise or problem set.** Bring your project code and questions. Reminder: the completed project is due via Canvas by 11:59 pm the following Sunday.

## `r session_headers[14]`

**Topics:** **Final Project Presentations (in class)**

The final project — R script, output, and written report — is due **via Canvas by 11:59 pm on Sunday, `r format(final_day - days(3), '%B %d, %Y')`**, the Sunday before this session. Presentations happen in class today. No late submissions, no appeals. Plan ahead.
...
|:---|:---:|:---|
| Homework, labs & in-class exercises | **45%** | Weekly problem sets (including the project checkpoints in Problem Sets 8–12) and the graded in-class exercises |
...
#| label: grading-table
#| echo: false
read_csv("data/grading-scheme.csv") |>
  kbl() |>
