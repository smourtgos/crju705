...
- Bayesian stack (**S6 changed Sep 21, 2026:** the course function `bayes_prop_test()`, proportions only, replaced BayesFactor there; S7, S10, and the midterm still assume BayesFactor and are not yet reconciled): ~~`BayesFactor` for t-tests (S6)~~ · `brms` + `emmeans` for ANOVA (S8) · `correlation` pkg (S10) · `brms` for regression/logistic (S11/S12) — matches Scott's 2025 workflow and the midterm
